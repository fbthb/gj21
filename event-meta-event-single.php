...
<div class="eventorganiser-event-meta">
	...
	<ul class="eo-event-meta">
		...
		<?php if ( ! eo_recurs() ) { ?>
			<!-- Single event -->
			<li><i class="far fa-calendar"></i><strong> <?php echo eo_format_event_occurrence();?></li>
		<?php } ?>

		<?php if ( eo_get_venue() ) {
			$tax = get_taxonomy( 'event-venue' ); ?>
			<li><i class="fas fa-map-marker-alt"></i> </strong> <a href="<?php eo_venue_link(); ?>"> <?php eo_venue_name(); ?></a></li>
		<?php } ?>

		<?php if ( get_the_terms( get_the_ID(), 'event-category' ) && ! is_wp_error( get_the_terms( get_the_ID(), 'event-category' ) ) ) { ?>
			<li><i class="far fa-folder-open"></i></strong> <?php echo get_the_term_list( get_the_ID(),'event-category', '', ', ', '' ); ?></li>
		<?php } ?>
	...
	<?php if ( 'none' !== eventorganiser_get_option('map_provider') && eo_get_venue() && eo_venue_has_latlng(eo_get_venue()) ) : ?>
		<!-- Display map -->
